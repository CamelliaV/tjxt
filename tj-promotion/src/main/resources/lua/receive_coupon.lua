-- 优惠劵数量存在
-- 对应用户领取量存在
-- redis 时间戳字符串转unix timestamp（s单位）
-- Define the timestamp
--local function timeToNumber(ts)
--    -- 字符串转table
...
--    --   print(k, v)
--    -- end
--    local mill = os.time(timeTable)
--    return tonumber(mill)
--end
-- * test
-- print(timeToNumber("2024-11-28 00:30:00"))
-- os.exit()
...
--[[
  ...
  3 - 判断发放时间 < begin || > end
  4 - 判断库存 <= 0
  5 - 判断已领取量increment 后 > userLimit(coupon)
  （无失败）减少totalNum
  0 - 成功
  -- MQ异步更新时延，查库后重查redis
]]--
...
-- 校验已领取数
-- hincrby返回修改后数据
if (tonumber(redis.call('HGET', couponKey, 'userLimit')) < redis.call('HINCRBY', userCouponKey, userId, '1')) then
